...
`notion-plat-mesh-pipe` is centered on platform engineering work in Java. The technical target is: Package a Java local lab for mesh analysis with append-only fixtures, checkpoint recovery checks, and documented operating limits.
...
The score is deterministic. The current threshold is 162. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 2.
